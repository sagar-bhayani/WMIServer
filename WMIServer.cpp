...
std::unique_ptr<WMIRecorder> g_httpDealer;

void on_initialize(const string_t& address)
{
    // Build our listener's URI from the configured address and the hard-coded path "WMIServer/recorder"

    uri_builder uri(address);
    uri.append_path(U("wmiserver/recorder"));

    auto addr = uri.to_uri().to_string();
    g_httpDealer = std::unique_ptr<WMIRecorder>(new WMIRecorder(addr));
